...
import { faAreaChart, faArrowLeft } from "@fortawesome/free-solid-svg-icons";
  ...
  const getMyCommands = async (id) => {
    try {
      ...
      const response = await ApiCall(
        `/api/v1/app/staff/commands/${token}/2`,
        "GET"
      );
      if (response?.error === false && response.data) {
      ...
      <div className="bg-white shadow rounded p-4">
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="text-green-600 text-lg">
            <FaCheck />
            {item.status === 2 && (
              <FaCheck className="inline ml-1 text-green-600" />
            )}
          </div>
        </div>
        ...
        <div className="flex items-center">
          <div className="w-full max-w-[1440px] mx-auto p-4">
            <h2 className="text-center mb-6 text-gray-600">
              <FontAwesomeIcon icon={faAreaChart} className="mr-2" />
              Jarayonda
            </h2>
            <input
              type="text"
