...
import { faInbox, faArrowLeft } from "@fortawesome/free-solid-svg-icons";
...
const NewCommand = () => {
      ...
      if (role === "ROLE_STAFF") {
        ...
        if (com.data.length > 0) {
          ...
          const resListStaff = await ApiCall(
            `/api/v1/app/commander/list-staff/${defaultCommander?.rank?.id}`,
            "GET"
          );
  ...
  return (
    <div className="flex">
      <Sidebar />
      <div
        className="bg-gray-200 p-4 sm:ml-64 w-full min-h-screen"
        style={{
        ...
        {isLoading ? (
          ...
          <div className="max-w-3xl mx-auto p-6 space-y-6">
            <h1 className="text-3xl text-gray-600 font-semibold text-center">
              <FontAwesomeIcon icon={faInbox} className="text-gray-600 mr-2" />
            ...
            {fileName && (
              <p className="text-sm text-gray-600">Fayl: {fileName}</p>
            )}
            ...
            {staffList.map((item) => (
              <div
                key={item.rank.id}
                className="border bg-gray-200 border-gray-500 p-3 rounded mb-2"
              >
                <div className="flex justify-between items-center">
                  <label className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      checked={item.staff.every(
                        (staff) => selectedStaff[staff.id]
                      )}
                      onChange={() =>
                        handleRankCheckboxToggle(item.rank.id, item.staff)
                      }
                    />
                    ...
                    {item.staff.map((staff) => (
                      <label key={staff.id} className="block">
                        <input
                          ...
                          onChange={() =>
                            handleStaffCheckboxToggle(item.rank.id, staff.id)
                          }
                        />
                        {staff.name}
                      </label>
