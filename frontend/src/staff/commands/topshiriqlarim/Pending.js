...
function PendingTopshiriq() {
  ...
  const getMyCommands = async (userId) => {
    try {
      ...
      const response = await ApiCall(
        `/api/v1/app/staff/my-commands/${token}/3`,
        "GET"
      );
  ...
  const handleNavigateToDetail = useCallback(
    (item) => {
      navigate("/mobil/commands/topshiriqlarim/javobtopshiriq", {
        state: { itemData: item },
      });
    },
    [navigate]
    ...
    return (
      <div key={item.id} className="bg-white shadow rounded p-4">
        ...
        <p className="text-gray-700 text-sm flex items-center">
          ...
          {`${tileLimitDate.toLocaleDateString(
            "en-GB"
          )}, ${tileLimitDate.toLocaleTimeString("en-GB", {
            hour: "2-digit",
            minute: "2-digit",
          })}`}
        </p>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <span
            className={`text-sm w-11/12 ${
              timeDifferenceInHours < 0 ? "text-red-500" : "text-black"
            }`}
          >
            {timeText}
          </span>
...
export default PendingTopshiriq;
