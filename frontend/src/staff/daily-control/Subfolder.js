...
function Subfolder() {
  ...
  const fetchData = async () => {
    try {
      ...
      const subfolderResponse = await ApiCall(
        `/api/v1/daily-control-subfolder/${subfolderId}`,
        "GET"
      );

      // Fetch contents
      const contentsResponse = await ApiCall(
        `/api/v1/daily-control-subfolder/content/${subfolderId}`,
        "GET"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        currentContent ? "Failed to update content" : "Failed to add content"
      );
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden mb-6">
              ...
              <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-4 flex justify-between items-center">
                ...
                >
                  <FontAwesomeIcon
                    icon={isCardCollapsed ? faChevronDown : faChevronUp}
                  />
                </button>
              </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-medium text-gray-800">
                        {subfolder.dailyControl.name}
                      </p>
                      <p className="text-gray-600">
                        {subfolder.dailyControl.description}
                      </p>
                    </div>
                  </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-medium flex items-center">
                            <FontAwesomeIcon
                              icon={getFileIcon(
                                content.filePath || content.linkToFile
                              )}
                          ...
                          {content?.attachment && (
                            <div className="ml-6 mt-2">
                              <button
                                className="text-blue-600 hover:underline flex items-center"
                                onClick={() =>
                                  downloadFile(content?.attachment)
                                }
                              >
                                <FontAwesomeIcon
                                  icon={faPaperclip}
                                  className="mr-1"
                                />
                                {content?.attachment?.name?.substring(
                                  content.attachment.name.indexOf("_") + 1
                                )}
                              </button>
                            </div>
                          )}
