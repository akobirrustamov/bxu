  ...
  const data = [
    { icon: "fa-university", value: 3, label: t("statistic.title1") },
    {
      ...
      subValues: [
        ...
        { value: 8, label: t("statistic.title2_text2") },
        { value: 2, label: t("statistic.title2_text3") },
      ],
    },
          ...
          <Zoom key={index}>
            ...
            >
              <div className="flex gap-4 items-center align-center justify-center">
                <i
                  className={`fa ${item.icon} text-6xl text-[#004C88]`}
                  aria-hidden="true"
                ></i>
                <h2>
                ...
                <div className="mt-4 flex flex-wrap justify-center gap-3">
                  {item.subValues.map((subItem, subIndex) => (
                    <div key={subIndex} className="text-[#004C88] text-xl">
                      <span>
                        {subItem.label}:{" "}
                        <CountUp start={0} end={subItem.value} duration={2} />
                      </span>
                    </div>
                  ))}
                </div>
